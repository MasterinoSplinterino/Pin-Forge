...
В проект зашиваются настройки процесса: слой, Arachne, компенсации, скорости,
ускорения, ironing, scarf joint. Имя профиля принтера пишется строкой
(`PRINTERS` в коде); если пресет называется иначе, настройки процесса
применятся, а принтер останется текущий.

Обдув и филамент намеренно не кладутся. Они относятся к пресету пластика,
и наполовину заполненный пресет приехал бы с температурами по умолчанию,
а это последнее, что стоит отдавать принтеру. Значения обдува лежат в разделе
«Рекомендации печати», чтобы вбить их в свой профиль филамента.

Чтобы слайсер вообще это прочитал, модель должна объявить себя проектом
семейства Bambu и Orca. Без этой метки файл импортируется голой геометрией,
а профиль молча выбрасывается.
